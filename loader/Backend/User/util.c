...
LPWSTR GetKeyPath(HKEY key) {
	if (key == NULL) return (LPWSTR)L"NULL_KEY";

	static WCHAR buffer[MAX_PATH] = { 0 };
	DWORD size = sizeof(buffer);
	memset(buffer, 0, sizeof(buffer));

	// NtQueryKey 3 is the Name Information class. 
	// We wrap this in a check to ensure we don't hang on volatile hardware keys.
	NTSTATUS status = NtQueryKey(key, 3, buffer, size, &size);

	if (status != 0) {
		return (LPWSTR)L"PROTECTED_KEY";
	}
...
VOID ForceDeleteFile(LPWSTR path) {
	...
	FreeSid(admin);
	LocalFree(acl);
} // This properly closes ForceDeleteFile

VOID RecursiveDelete(LPWSTR dir, LPWSTR match) {
	WCHAR searchPath[MAX_PATH];
	swprintf(searchPath, MAX_PATH, L"%s\\*", dir);

	ForEachFile(searchPath, {
		WCHAR fullPath[MAX_PATH];
		swprintf(fullPath, MAX_PATH, L"%s\\%s", dir, file);

		// If the filename contains our match string (e.g., "log" or "cache")
		if (StrStrI(file, match)) {
